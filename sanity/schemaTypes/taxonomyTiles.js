import {defineField, defineType} from 'sanity'

// CS2-specific bespoke block: 5-tile capabilities taxonomy.
// Even split across the available width with vertical rules between tiles.
export default defineType({
  name: 'taxonomyTiles',
  title: 'Taxonomy Tiles (5-up)',
  type: 'object',
  fields: [
    defineField({
      name: 'tiles',
      title: 'Tiles',
      type: 'array',
      of: [
        {
          type: 'object',
          name: 'tile',
          fields: [
            {name: 'number', type: 'string', title: 'Number (e.g. "01")'},
            {name: 'title', type: 'string', title: 'Title', validation: (Rule) => Rule.required()},
            {name: 'body', type: 'text', rows: 3, title: 'Body', validation: (Rule) => Rule.required()},
          ],
          preview: {
            select: {title: 'title', subtitle: 'body'},
          },
        },
      ],
      validation: (Rule) => Rule.required().length(5),
    }),
  ],
  preview: {
    select: {tiles: 'tiles'},
    prepare: ({tiles}) => ({
      title: 'Taxonomy Tiles',
      subtitle: tiles?.map((t) => t.title).join(' · ') || '',
    }),
  },
})
